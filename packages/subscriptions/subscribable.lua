  ...
  package.preload["storage-vanilla"] = function(...)
    ...
    mod.Subscribers :
    {
      ...
      function mod.unsubscribeFromTopics(processId, topics)
        print("unsubscribeFromTopics:" .. processId .. " " .. json.encode(topics))
        local existingTopics = json.decode(mod.Subscribers[processId].topics)
        local newTopics = {}
        for _, topic in ipairs(existingTopics) do
          local shouldKeep = true
          for _, topicToRemove in ipairs(topics) do
            if topic == topicToRemove then
              shouldKeep = false
              break
            end
          end
        ...
        print("newTopics:" .. json.encode(newTopics))
        mod.Subscribers[processId].topics = json.encode(newTopics)
      end
...
local function newmodule(cfg)
  ...
  pkg.version = '2.0.1'     -- requires an aos 2.0 process, uses msg.reply() etc.
